...
А для чего остальные методы? Метод <code>push</code> записывает новые элементы в конец массива. Он подходит для добавление элементов в стек, а не получения.
...
А что с методом <code>push</code>? Он записывает новые элементы в конец массива и подходит для добавление элементов в стек, а не получения.`
    },
    {
      isCorrect: false,
...
export default {
  ...
  description: () => {
    return [
      ...
      '',
      'По сути это разные способы хранить информацию. И этих способов много. У каждой структуры есть свои особенности — она может быть эффективна в в одних случаях и не очень эффективна в других. Задача разработчика — выбрать наиболее подходящую структуру данных для задачи.',
      '',
      'Обычно этим занимаются не начинающие разработчики, а ребята с уровнем middle и senior. Но про структуры данных часто спрашивают на собеседованиях. А понимание того, как эффективно можно хранить данные, даст вам +100 очков к карьерному развитию.',
      '',
